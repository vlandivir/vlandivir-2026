...
| [gtd/gtd-api.controller.ts](../src/gtd/gtd-api.controller.ts) | `/gtd-api` | Private GTD API for projects, one-task queue, snoozing, archive, history, private attachments, sync list endpoints (`GET /tasks`, `GET /projects`) and optional Google ↔ Telegram account linking; accepts Google session/Bearer or signed Telegram initData |
...
| `email/` | Mail UI | Google session; IMAP ingest on the server |
